...
* 但新手面对众多选择无从下手，总是求助于使用过的非面向对象技术。
    * 并不是解决任何问题都需要从头做起的；
    * 当找到一个好的解决方案时，我们可以一遍又一遍地使用，在节省研发开销的同时，对该解决方案也进行更好地维护。
* 总结设计经验的重要价值：
...
### **单一职责原则**
* 一个类应该只有一个职责，只有一个引起它变化的原因
* 每个职责都是变化的一个轴线
* 如一个类有一个以上的职责：职责耦合，导致脆弱的设计
    * 影响复用性
    * 当一个职责发生变化时，可能影响其他的职责

### **开闭原则**（Open Close Principle）
* 对扩展开放，对修改关闭
* 程序需要拓展时，不能修改原有的代码，应实现热插拔的效果
* 提高扩展性，易于维护和升级
* 使用接口和抽象类
...
表格注释：  
* 创造型类模式：将对象部分创建工作延迟到子类
* 创造型对象模式：将对象部分创建工作延迟到另一个对象中
* 结构型类模式：使用继承机制组合类
* 结构型对象模式：描述对象的组装方式
* 行为型类模式：使用继承描述算法和控制流
